...
let AiAnalyticsService = AiAnalyticsService_1 = class AiAnalyticsService {
    ...
    async getDropoutRiskStudents(tenantId, schoolId) {
    ...
    async getPerformancePrediction(tenantId, studentId) {
    ...
    async getAttendanceAnalytics(tenantId, schoolId) {
        const where = { tenantId, ...(schoolId && { student: { schoolId } }) };
        ...
        return { overallRate: Math.round((present / total) * 100), byStatus: byStatus.reduce((acc, b) => { acc[b.status] = b._count; return acc; }, {}), dailyTrend: daily30, totalRecords: total };
    }
    async getFeeAnalytics(tenantId) {
        ...
        return { collectionRate: total > 0 ? Math.round((paid / total) * 100) : 0, totalBilled: total, totalCollected: paid, outstanding: total - paid, byStatus: byStatus.reduce((acc, b) => { acc[b.status] = { count: b._count, amount: b._sum.amount }; return acc; }, {}), monthlyRevenue, overdueTrend };
    }
    async getSchoolPerformanceDashboard(tenantId) {
    ...
    async getBenchmarkingData(tenantId) {
    ...
    async generateAiReport(type, tenantId, params) {
        const reports = {
    ...
    generateInsights(type, data) {
        if (type === 'attendance')
            return [`Overall attendance rate: ${data.overallRate}%`, `${data.byStatus?.ABSENT ?? 0} absences recorded`, data.overallRate < 80 ? '⚠️ Attendance below target (85%)' : '✅ Attendance meeting target'];
        if (type === 'fees')
            return [`Collection rate: ${data.collectionRate}%`, `Outstanding: ${data.outstanding?.toLocaleString()}`, data.collectionRate < 70 ? '⚠️ Fee collection needs improvement' : '✅ Good collection rate'];
        return ['Analysis complete', 'Data processed successfully'];
    }
    generateRecommendations(type, data) {
        if (type === 'attendance' && data.overallRate < 80)
            return ['Enable SMS alerts for absent students', 'Implement QR code attendance for faster marking', 'Schedule parent meetings for chronic absentees'];
        if (type === 'fees' && data.collectionRate < 70)
            return ['Send automated fee reminders 7 days before due date', 'Offer installment payment plans', 'Enable online payment gateway'];
