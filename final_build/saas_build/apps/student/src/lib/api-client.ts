...
http.interceptors.request.use(cfg => {
  if (typeof window !== 'undefined') {
    ...
    const slug  = localStorage.getItem('student_tenant_slug');
...
http.interceptors.response.use(
  ...
  err => {
    if (err.response?.status === 401 && typeof window !== 'undefined') {
      ...
      window.location.href = `/login?school=${slug}`;
    }
...
export function setTokens(token: string, slug: string) {
  ...
  localStorage.setItem('student_tenant_slug', slug);
}

export function clearTokens() {
  localStorage.removeItem('student_access_token');
  localStorage.removeItem('student_tenant_slug');
}

export function getSlug(): string {
  return typeof window !== 'undefined' ? localStorage.getItem('student_tenant_slug') || '' : '';
}
