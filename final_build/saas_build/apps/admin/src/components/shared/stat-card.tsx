...
export function StatCard({ title, value, icon, trend, color='green' }: { title:string; value:string|number; icon:string; trend?:{value:number;label:string}; color?:string }) {
    ...
    <div className={`${c.bg} rounded-xl p-5 border shadow-sm hover:shadow-md transition-shadow`}>
      <div className="flex items-start justify-between">
        <div>
          ...
          {trend && <p className={`text-xs font-semibold mt-2 ${trend.value>=0?'text-green-600':'text-red-500'}`}>{trend.value>=0?'↑':'↓'} {Math.abs(trend.value)}% {trend.label}</p>}
        </div>
