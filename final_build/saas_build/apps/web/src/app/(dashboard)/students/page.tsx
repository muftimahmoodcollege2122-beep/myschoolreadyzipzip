...
const INIT = {
  firstName: '', lastName: '', email: '', admissionNo: '',
  rollNumber: '', phone: '', gender: 'MALE',
...
export default function StudentsPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErr('');
    if (!form.admissionNo) { setErr('Admission No is required.'); return; }
    if (!form.academicYear) { setErr('Academic Year is required.'); return; }
    try {
      await create.mutateAsync(form);
      setModal(false);
      setForm(INIT);
    } catch (e: any) {
      const msg = e?.response?.data?.message ?? e?.message ?? 'Failed to create student.';
    ...
    <>
      ...
      <div className="p-6">
        <PageHeader title="Students" subtitle={`${data?.meta.total ?? 0} enrolled`}
          action={<button onClick={() => { setErr(''); setForm(INIT); setModal(true); }} className="px-4 py-2 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-500 transition-colors">+ Add Student</button>} />
          ...
          <form onSubmit={submit} className="grid grid-cols-2 gap-4">
            ...
            {FIELDS.map(([key, label, type]) => (
              <div key={key}>
                ...
                <input type={type} value={(form as any)[key] ?? ''} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-400" />
              </div>
            ))}
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Gender</label>
              <select value={form.gender} onChange={e => setForm(f => ({ ...f, gender: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:border-green-400">
