...
const PLANS = [
  { name: 'Starter', price: '4,999', students: 'Up to 500 students', features: ['School website', 'Admin portal', 'Attendance & fees', 'Email support'] },
  { name: 'Professional', price: '12,999', students: 'Up to 2,000 students', recommended: true, features: ['All Starter features', 'LMS & courses', 'Teacher & parent portals', 'AI Assistant', 'SMS integration'] },
  { name: 'Enterprise', price: '29,999', students: 'Unlimited students', features: ['All Pro features', 'White-label branding', 'Custom domain', 'API access', 'Dedicated SLA'] },
];
...
export default function SignupPage() {
  ...
  const suggestedDomain = form.schoolName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').slice(0, 30) || 'yourschool';
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-lg w-full">
          ...
          <p className="text-gray-500 text-center text-sm mb-7">Your complete school management system has been provisioned. Login credentials have been sent to your email.</p>

          <div className="bg-gray-50 rounded-xl p-5 mb-6 space-y-3 border border-gray-100">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-mono font-bold text-gray-900 bg-yellow-50 border border-yellow-200 px-2.5 py-1 rounded-lg">{result.tempPassword}</span>
            </div>
          </div>

          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-xs text-amber-800">
            <p className="font-bold mb-1">Important — change your password after first login</p>
            <p className="text-amber-700">Your school portal is running on a 30-day free trial with full access to all features.</p>
          </div>

          <Link href="/login" className="block w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl text-center text-sm transition-colors">
            Go to Admin Portal →
          </Link>
          <p className="text-center text-xs text-gray-400 mt-3">Credentials also sent to {result.adminEmail}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-white/10 p-10 max-w-md w-full" style={{ background: '#0F2D50' }}>
          <div className="text-center mb-8">
            <div className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <svg className="w-7 h-7 text-white animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
            </div>
          ...
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${(provStep / PROV_STEPS.length) * 100}%` }} />
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Left panel */}
      <div className="hidden lg:flex w-[42%] flex-col justify-between p-12" style={{ background: 'linear-gradient(150deg, #0C1E35, #0F3D6E)' }}>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex items-center gap-2 mb-8">
            {STEPS.map((s, i) => (
              <React.Fragment key={s}>
                ...
                {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 rounded-full transition-all ${i < step - 1 ? 'bg-blue-500' : 'bg-gray-200'}`} />}
              </React.Fragment>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
              ...
              <>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input value={form.schoolName} onChange={e => set('schoolName', e.target.value)} placeholder="e.g. Beacon House School System" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
                  </div>
                  <div>
                    ...
                    <input value={form.principalName} onChange={e => set('principalName', e.target.value)} placeholder="Dr. Ahmed Khan" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Country</label>
                    <select value={form.country} onChange={e => set('country', e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-white outline-none focus:border-blue-400 transition-all">
                  ...
                  <div>
                    ...
                    <select value={form.studentCount} onChange={e => set('studentCount', e.target.value)} className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm bg-white outline-none focus:border-blue-400 transition-all">
              ...
              <>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="principal@yourschool.com" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
                  </div>
                  <div>
                    ...
                    <input value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+92-300-1234567" className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all" />
                  </div>
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm">
                    <p className="font-bold text-blue-800 mb-2 text-xs">After provisioning, we automatically send:</p>
                    <ul className="space-y-1 text-xs text-blue-700">
                      <li className="flex items-center gap-2"><span className="text-blue-400">•</span>Admin portal login URL</li>
                      <li className="flex items-center gap-2"><span className="text-blue-400">•</span>Username &amp; temporary password</li>
                      <li className="flex items-center gap-2"><span className="text-blue-400">•</span>School website link</li>
                      <li className="flex items-center gap-2"><span className="text-blue-400">•</span>Onboarding checklist &amp; guide</li>
                    </ul>
                  </div>
                </div>
              </>
              ...
              <>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <p className="text-xs font-bold text-gray-700 mb-2">Custom Domain (Optional — set up after registration)</p>
                    <div className="flex items-center gap-2 px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-xs text-gray-400">
                      <span>www.yourschool.com</span>
                    </div>
                    <p className="text-xs text-gray-400 mt-1.5">Connect your own domain in Settings → Domain after login.</p>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-1 text-blue-700">
                      {['Homepage', 'About Us', 'Admissions', 'Staff Directory', 'News & Events', 'Gallery', 'Contact Form', 'SEO Optimized'].map(p => (
                        <span key={p} className="flex items-center gap-1">
                          <svg className="w-3 h-3 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>
                          {p}
                        </span>
                      ))}
                    </div>
            ...
            {step === 4 && (
              <>
                  ...
                  {PLANS.map(p => (
                    <label key={p.name} className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${form.plan === p.name ? 'border-blue-500 bg-blue-50' : 'border-gray-100 hover:border-gray-200'}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mb-2">{p.students}</p>
                ...
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3.5 text-sm">
                  <p className="font-bold text-emerald-800 text-xs">30-day free trial included — full access, no card required</p>
                </div>
              </>
            )}
            ...
            <div className="flex gap-3 mt-6">
              {step > 1 && (
                <button onClick={() => { setStep(s => s - 1); setError(''); }} className="flex-1 py-3 border border-gray-200 text-gray-600 font-semibold rounded-xl hover:bg-gray-50 text-sm transition-all">
                  ← Back
                </button>
              )}
              <button
