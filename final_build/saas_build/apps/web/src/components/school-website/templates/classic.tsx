    ...
    <div style={{ background: theme.bgColor, minHeight: '100vh' }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${theme.primaryColor} 0%, ${theme.secondaryColor} 100%)`, padding: '120px 0 80px', textAlign: 'center' }}>
        <div className="container">
          ...
          <h1 style={{ color: '#fff', fontSize: 'clamp(2rem,5vw,3.5rem)', fontFamily: 'var(--font-heading)', fontWeight: 800, margin: '0 0 16px', lineHeight: 1.1 }}>{theme.schoolName}</h1>
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '1.15rem', maxWidth: 600, margin: '0 auto 32px' }}>{theme.tagline}</p>
        ...
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
          <div>
            ...
            <p style={{ color: '#4A5E6D', lineHeight: 1.8, marginBottom: 16 }}>
              {theme.schoolName} is dedicated to providing world-class education in {theme.city}.
              Our experienced faculty and modern facilities ensure every student reaches their full potential.
            </p>
            ...
            <button className="btn-primary" style={{ marginTop: 24 }} onClick={() => setShowAdmission(true)}>Enroll Today</button>
          </div>
