...
http.interceptors.request.use(cfg => {
  if (typeof window !== 'undefined') {
    ...
    const slug  = localStorage.getItem('teacher_tenant_slug');
...
export function setTokens(token: string, slug: string) {
  ...
  localStorage.setItem('teacher_tenant_slug', slug);
}

export function clearTokens() {
  localStorage.removeItem('teacher_access_token');
  localStorage.removeItem('teacher_tenant_slug');
}

export function getSlug(): string {
  return typeof window !== 'undefined' ? localStorage.getItem('teacher_tenant_slug') || '' : '';
}
