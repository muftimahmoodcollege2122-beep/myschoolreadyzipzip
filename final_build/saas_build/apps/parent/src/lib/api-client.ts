...
http.interceptors.request.use(cfg => {
  if (typeof window !== 'undefined') {
    ...
    const slug  = localStorage.getItem('parent_tenant_slug');
...
export function setTokens(token: string, slug: string) {
  ...
  localStorage.setItem('parent_tenant_slug', slug);
}

export function clearTokens() {
  localStorage.removeItem('parent_access_token');
  localStorage.removeItem('parent_tenant_slug');
}

export function getSlug(): string {
  return typeof window !== 'undefined' ? localStorage.getItem('parent_tenant_slug') || '' : '';
}
