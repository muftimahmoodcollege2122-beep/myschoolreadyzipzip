    ...
    <div style={{ background: theme.secondaryColor, minHeight: '100vh' }}>
      ...
      <section style={{ minHeight: '100vh', background: `linear-gradient(180deg, ${theme.secondaryColor} 0%, ${theme.primaryColor} 100%)`, display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ color: '#fff', fontSize: 'clamp(3rem,8vw,7rem)', fontFamily: 'var(--font-heading)', fontWeight: 900, lineHeight: 0.95, margin: '0 0 24px', textTransform: 'uppercase', letterSpacing: '-0.02em' }}>{theme.schoolName}</h1>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.2rem', maxWidth: 500, marginBottom: 40, lineHeight: 1.7 }}>{theme.tagline}</p>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <button className="btn-primary" style={{ background: theme.accentColor, fontSize: '1rem', padding: '16px 36px' }} onClick={() => setShowAdmission(true)}>APPLY NOW</button>
